...
class Clock extends Component {
    ...
    initOrRefersh(isInit = true) {
        ...
        if(isInit) {
            this.state = {datetime, suspend : false};
        } else {
            // 会与原来的 state 进行合并
    ...
    render() {
        return <div>
            <h1 style={ {cursor: 'pointer', color : this.state.suspend ? '#EEEEEE' : '#000000'} } onClick={ this.handleClick2.bind(this) }>{ this.state.datetime }</h1>
